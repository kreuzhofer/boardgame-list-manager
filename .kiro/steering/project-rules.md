...
Risk-Scaled Rigor (Lightweight): Scale rigor with impact: 1) Low risk: unit tests, lint/format. 2) Medium risk: integration tests, edge cases, rollback awareness. 3) High risk (security, auth, money, data loss, core flows): explicit approval before destructive actions, targeted tests, minimal refactoring.