...
describe('SSE Event Payload Properties', () => {
  /**
   * Property 2: Event Payload Structure
   * For any SSE event broadcast, the payload SHALL contain `type`, `gameId`, and `participantId` fields.
   */
  describe('Property 2: Event Payload Structure', () => {
    it('all events contain required base fields (type, gameId, participantId)', () => {
      fc.assert(
        fc.property(gameEventArb, (event) => {
          // All events must have type, gameId, participantId
    ...
    it('game:created events include participantName, gameName, and isBringing', () => {
      fc.assert(
        fc.property(gameCreatedEventArb, (event) => {
          expect(event.type).toBe('game:created');
          // Cast to access type-specific fields
          const createdEvent = event as { participantName: string; gameName: string; isBringing: boolean };
          expect(createdEvent.participantName).toBeDefined();
          expect(typeof createdEvent.participantName).toBe('string');
    ...
    it('game:bringer-added events include participantName and gameName', () => {
      fc.assert(
        fc.property(bringerAddedEventArb, (event) => {
          expect(event.type).toBe('game:bringer-added');
          // Cast to access type-specific fields
          const bringerEvent = event as { participantName: string; gameName: string };
          expect(bringerEvent.participantName).toBeDefined();
          expect(typeof bringerEvent.participantName).toBe('string');
    ...
    it('JSON serialization round-trip preserves all fields', () => {
      fc.assert(
        fc.property(gameEventArb, (event) => {
          ...
          expect(deserialized.participantId).toBe(event.participantId);

          // Type-specific fields preserved
          if (event.type === 'game:created') {
            expect(deserialized.participantName).toBe(event.participantName);
